...
class GenerateSponsorLogoUploadURL(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        ext = request.data.get('ext', 'jpg')
        key = f"sponsors/logos/{uuid.uuid4()}.{ext}"
        s3 = boto3.client(
            's3',
            endpoint_url=settings.AWS_S3_ENDPOINT_URL,
            aws_access_key_id=settings.AWS_ACCESS_KEY_ID,
        ...
        try:
            upload_url = s3.generate_presigned_url(
                ...
                Params={
                    'Bucket': settings.AWS_STORAGE_BUCKET_NAME,
                    'Key': key,
                    'ContentType': request.data.get('contentType', 'image/png'),
                },
                ExpiresIn=600,
                HttpMethod='PUT',
            )
...
# --- News Article Cover Upload ---
class GenerateNewsArticleCoverUploadURL(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        ext = request.data.get('ext', 'jpg')
        key = f"news-covers/{uuid.uuid4()}.{ext}"
        s3 = boto3.client(
            's3',
            endpoint_url=settings.AWS_S3_ENDPOINT_URL,
            aws_access_key_id=settings.AWS_ACCESS_KEY_ID,
...
class GenerateTeamGalleryPhotoUploadURL(APIView):
    permission_classes = [IsAuthenticated]

    def post(self, request):
        ext = request.data.get('ext', 'jpg')
        key = f"team-gallery/{uuid.uuid4()}.{ext}"
        s3 = boto3.client(
            's3',
            endpoint_url=settings.AWS_S3_ENDPOINT_URL,
            aws_access_key_id=settings.AWS_ACCESS_KEY_ID,
